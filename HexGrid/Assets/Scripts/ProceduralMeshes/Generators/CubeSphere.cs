...
namespace ProceduralMeshes.Generators
{
    public struct CubeSphere : IMeshGenerator
    {
        ...
        public void Execute<S>(int i, S streams) where S : struct, IMeshStreams
        {
            ...
            float3 uA = CubeToSphere(side.uvOrigin + side.uVector * u / Resolution);
            float3 uB = CubeToSphere(side.uvOrigin + side.uVector * (u + 1) / Resolution);
            float3 pA = uA;
            float3 pB = uB;
        ...
        {
            0 => new Side { id = id, uvOrigin = -1f, uVector = 2f * right(), vVector = 2f * up(), seamStep = 4 },
            1 => new Side { id = id, uvOrigin = float3(1f, -1f, -1f), uVector = 2f * forward(), vVector = 2f * up(), seamStep = 4 },
            2 => new Side { id = id, uvOrigin = float3(-1f, -1f, -1f), uVector = 2f * forward(), vVector = 2f * right(), seamStep = -2 },
            3 => new Side { id = id, uvOrigin = float3(-1f, -1f, 1f), uVector = 2f * up(), vVector = 2f * right(), seamStep = -2 },
            4 => new Side { id = id, uvOrigin = float3(-1f, -1f, -1f), uVector = 2f * up(), vVector = 2f * forward(), seamStep = -2 },
            _ => new Side { id = id, uvOrigin = float3(-1f, 1f, -1f), uVector = 2f * right(), vVector = 2f * forward(), seamStep = -2 }
